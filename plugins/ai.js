...
module.exports = function() {
    return function (irc) {
        ...
        function runAI(channel) {
            var offset = (24 * 60 * 60 * 1000) * cfg.ai.dateThreshold,
                dateObj = new Date(),
                query;
            ...
            dbt.count(query, function(err, count) {
                ...
                dbt.find(query, { limit: 1, skip: rand }, function(err, res) {
                    if (typeof res[0] === 'object') {
                        // SPLURT
                        irc.send(channel, processOutgoing(channel, res[0].message));
                    }
        ...
        irc.on('welcome', function() {
            for (var i = cfg.server.join.length - 1; i >= 0; i--) {
                var channel = cfg.server.join[i];
                setInterval(function() {
                    irc.names(channel, function(err, names) {
